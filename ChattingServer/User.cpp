...
//	list_packet.id = userId;
//	list_packet.type = ROOM_LIST;
//	list_packet.roomCnt = Singleton::GetInstance()->roomList.size();
//	SendPacket(reinterpret_cast<unsigned char*>(&list_packet));
//
//}
//
//// 채팅룸 메시지 보내는 함수
//void User::SendRoomChatting(int target, char* msg)
//{
//	Room_Chatting chat_packet;
//
//	chat_packet.id = userId;
//	chat_packet.type = ROOM_CHATTING;
//	chat_packet.size = sizeof(Room_Chatting);
...
//		Singleton::GetInstance()->channel[userInfo->channel_index].AddNewRoom(packet->roomIndex, newRoom); // 채널에 방 추가
//
...
//
//
//	for (auto iter : ChattingServer::GetInstance()->GetAllUser())
//	{
//		if (iter->GetChannelIndex() == userInfo->GetChannelIndex())		// 채널이 같고
//		{
//			if (iter->GetRoomIndex() == userInfo->GetRoomIndex())		// 방이 같아야 채팅을 보낸다.
//			{
//				
//			}
//		}
